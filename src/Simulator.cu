...
void Simulator::setup()
{
  ...
  std::cout << "Maximum (theoretical) number of cells: " << maxNumCells << ", actual grid dimensions: " << gridDim.x
  ...
  CURAND_CALL(curandGenerateNormalDouble(generator, ddps[(uint32_t)DDP::R], numBubbles, avgRad, stdDevRad));
  CURAND_CALL(curandDestroyGenerator(generator));

  KERNEL_LAUNCH(assignDataToBubbles, kernelSize, 0, 0, ddps[(uint32_t)DDP::X], ddps[(uint32_t)DDP::Y],
                ddps[(uint32_t)DDP::Z], ddps[(uint32_t)DDP::XP], ddps[(uint32_t)DDP::YP], ddps[(uint32_t)DDP::ZP],
                ddps[(uint32_t)DDP::R], ddps[(uint32_t)DDP::RP], dips[(uint32_t)DIP::FLAGS], bubblesPerDim, tfr, lbb,
                avgRad, properties.getMinRad(), numBubbles);

  cubWrapper.reduceNoCopy<double, double *, double *>(&cub::DeviceReduce::Sum, ddps[(uint32_t)DDP::RP], dasai,
                                                      numBubbles, 0);
  CUDA_CALL(cudaMemcpyAsync(static_cast<void *>(ddps[(uint32_t)DDP::RP]), static_cast<void *>(ddps[(uint32_t)DDP::R]),
                            sizeof(double) * dataStride, cudaMemcpyDeviceToDevice, 0));

  // Delete small bubbles, if any
  const int numBubblesAboveMinRad =
    cubWrapper.reduce<int, int *, int *>(&cub::DeviceReduce::Sum, dips[(uint32_t)DIP::FLAGS], numBubbles);
  if (numBubblesAboveMinRad < numBubbles)
    deleteSmallBubbles(numBubblesAboveMinRad);

  maxBubbleRadius =
    cubWrapper.reduce<double, double *, double *>(&cub::DeviceReduce::Max, ddps[(uint32_t)DDP::R], numBubbles);
  ...
  KERNEL_LAUNCH(resetKernel, kernelSize, 0, 0, 0.0, numBubbles, ddps[(uint32_t)DDP::DXDTO], ddps[(uint32_t)DDP::DYDTO],
                ddps[(uint32_t)DDP::DZDTO], ddps[(uint32_t)DDP::DRDTO], ddps[(uint32_t)DDP::DISTANCE],
                ddps[(uint32_t)DDP::PATH]);

  std::cout << "Calculating some initial values as a part of setup." << std::endl;

  if (NUM_DIM == 3)
  {
    KERNEL_LAUNCH(velocityPairKernel, pairKernelSize, 0, 0, properties.getFZeroPerMuZero(), dips[(uint32_t)DIP::PAIR1],
                  dips[(uint32_t)DIP::PAIR2], ddps[(uint32_t)DDP::R], interval.x, lbb.x, PBC_X == 1,
                  ddps[(uint32_t)DDP::X], ddps[(uint32_t)DDP::DXDTO], interval.y, lbb.y, PBC_Y == 1,
                  ddps[(uint32_t)DDP::Y], ddps[(uint32_t)DDP::DYDTO], interval.z, lbb.z, PBC_Z == 1,
    ...
    doBoundaryWrap(kernelSize, 0, 0, PBC_X == 1, PBC_Y == 1, PBC_Z == 1, numBubbles, ddps[(uint32_t)DDP::XP],
                   ddps[(uint32_t)DDP::YP], ddps[(uint32_t)DDP::ZP], lbb, tfr, dips[(uint32_t)DIP::WRAP_COUNT_X],
                   dips[(uint32_t)DIP::WRAP_COUNT_Y], dips[(uint32_t)DIP::WRAP_COUNT_Z],
                   dips[(uint32_t)DIP::WRAP_COUNT_XP], dips[(uint32_t)DIP::WRAP_COUNT_YP],
  ...
  else
  {
    KERNEL_LAUNCH(velocityPairKernel, pairKernelSize, 0, 0, properties.getFZeroPerMuZero(), dips[(uint32_t)DIP::PAIR1],
                  dips[(uint32_t)DIP::PAIR2], ddps[(uint32_t)DDP::R], interval.x, lbb.x, PBC_X == 1,
                  ddps[(uint32_t)DDP::X], ddps[(uint32_t)DDP::DXDTO], interval.y, lbb.y, PBC_Y == 1,
                  ddps[(uint32_t)DDP::Y], ddps[(uint32_t)DDP::DYDTO]);

    KERNEL_LAUNCH(eulerKernel, kernelSize, 0, 0, numBubbles, timeStep, ddps[(uint32_t)DDP::X],
                  ddps[(uint32_t)DDP::DXDTO], ddps[(uint32_t)DDP::Y], ddps[(uint32_t)DDP::DYDTO]);

    doBoundaryWrap(kernelSize, 0, 0, PBC_X == 1, PBC_Y == 1, false, numBubbles, ddps[(uint32_t)DDP::XP],
                   ddps[(uint32_t)DDP::YP], ddps[(uint32_t)DDP::ZP], lbb, tfr, dips[(uint32_t)DIP::WRAP_COUNT_X],
                   dips[(uint32_t)DIP::WRAP_COUNT_Y], dips[(uint32_t)DIP::WRAP_COUNT_Z],
                   dips[(uint32_t)DIP::WRAP_COUNT_XP], dips[(uint32_t)DIP::WRAP_COUNT_YP],
                   dips[(uint32_t)DIP::WRAP_COUNT_ZP]);
...
double Simulator::stabilize()
{
  ...
  if (NUM_DIM == 3)
    KERNEL_LAUNCH(potentialEnergyKernel, pairKernelSize, 0, 0, numBubbles, dips[(uint32_t)DIP::PAIR1],
                  dips[(uint32_t)DIP::PAIR2], ddps[(uint32_t)DDP::R], ddps[(uint32_t)DDP::TEMP4], interval.x,
                  PBC_X == 1, ddps[(uint32_t)DDP::X], interval.y, PBC_Y == 1, ddps[(uint32_t)DDP::Y], interval.z,
                  PBC_Z == 1, ddps[(uint32_t)DDP::Z]);
  else
    KERNEL_LAUNCH(potentialEnergyKernel, pairKernelSize, 0, 0, numBubbles, dips[(uint32_t)DIP::PAIR1],
                  dips[(uint32_t)DIP::PAIR2], ddps[(uint32_t)DDP::R], ddps[(uint32_t)DDP::TEMP4], interval.x,
                  PBC_X == 1, ddps[(uint32_t)DDP::X], interval.y, PBC_Y == 1, ddps[(uint32_t)DDP::Y]);

  cubWrapper.reduceNoCopy<double, double *, double *>(&cub::DeviceReduce::Sum, ddps[(uint32_t)DDP::TEMP4], dtfapr,
                                                      numBubbles);
  CUDA_CALL(cudaMemcpyAsync(static_cast<void *>(&pinnedDoubles[1]), static_cast<void *>(dtfapr), sizeof(double),
  ...
  for (int i = 0; i < properties.getNumStepsToRelax(); ++i)
  {
    do
    {
      if (NUM_DIM == 3)
      {
        KERNEL_LAUNCH(resetKernel, kernelSize, 0, 0, 0.0, numBubbles, ddps[(uint32_t)DDP::DXDTP],
                      ddps[(uint32_t)DDP::DYDTP], ddps[(uint32_t)DDP::DZDTP], ddps[(uint32_t)DDP::ERROR],
        ...
        KERNEL_LAUNCH(predictKernel, kernelSize, 0, 0, numBubbles, timeStep, ddps[(uint32_t)DDP::XP],
                      ...
                      ddps[(uint32_t)DDP::DZDT], ddps[(uint32_t)DDP::DZDTO]);
        ...
        KERNEL_LAUNCH(velocityPairKernel, pairKernelSize, 0, 0, properties.getFZeroPerMuZero(),
                      ...
                      PBC_Y == 1, ddps[(uint32_t)DDP::YP], ddps[(uint32_t)DDP::DYDTP], interval.z, lbb.z, PBC_Z == 1,
        ...
        KERNEL_LAUNCH(correctKernel, kernelSize, 0, 0, numBubbles, timeStep, ddps[(uint32_t)DDP::ERROR],
                      ...
                      ddps[(uint32_t)DDP::DXDTP], ddps[(uint32_t)DDP::YP], ddps[(uint32_t)DDP::Y],
                      ddps[(uint32_t)DDP::DYDT], ddps[(uint32_t)DDP::DYDTP], ddps[(uint32_t)DDP::ZP],
                      ddps[(uint32_t)DDP::Z], ddps[(uint32_t)DDP::DZDT], ddps[(uint32_t)DDP::DZDTP]);
      }
      else // Two dimensional case
      {
        KERNEL_LAUNCH(resetKernel, kernelSize, 0, 0, 0.0, numBubbles, ddps[(uint32_t)DDP::DXDTP],
                      ddps[(uint32_t)DDP::DYDTP], ddps[(uint32_t)DDP::ERROR], ddps[(uint32_t)DDP::TEMP1],
        ...
        KERNEL_LAUNCH(correctKernel, kernelSize, 0, 0, numBubbles, timeStep, ddps[(uint32_t)DDP::ERROR],
                      ddps[(uint32_t)DDP::XP], ddps[(uint32_t)DDP::X], ddps[(uint32_t)DDP::DXDT],
                      ddps[(uint32_t)DDP::DXDTP], ddps[(uint32_t)DDP::YP], ddps[(uint32_t)DDP::Y],
                      ddps[(uint32_t)DDP::DYDT], ddps[(uint32_t)DDP::DYDTP]);
      }
      ...
      // Error
      error =
        cubWrapper.reduce<double, double *, double *>(&cub::DeviceReduce::Max, ddps[(uint32_t)DDP::ERROR], numBubbles);
    ...
    CUDA_CALL(
      cudaMemcpyAsync(ddps[(uint32_t)DDP::X], ddps[(uint32_t)DDP::XP], numBytesToCopy, cudaMemcpyDeviceToDevice, 0));
  ...
  energy2 =
    cubWrapper.reduce<double, double *, double *>(&cub::DeviceReduce::Sum, ddps[(uint32_t)DDP::TEMP4], numBubbles);
...
bool Simulator::integrate()
{
  ...
  do
  {
    ...
    if (NUM_DIM == 3)
    {
      // Reset
      KERNEL_LAUNCH(resetKernel, kernelSize, 0, 0, 0.0, numBubbles, ddps[(uint32_t)DDP::DXDTP],
                    ddps[(uint32_t)DDP::DYDTP], ddps[(uint32_t)DDP::DZDTP], ddps[(uint32_t)DDP::DRDTP],
                    ddps[(uint32_t)DDP::ERROR], ddps[(uint32_t)DDP::TEMP1], ddps[(uint32_t)DDP::TEMP2]);
      ...
      // Velocity
      KERNEL_LAUNCH(velocityPairKernel, pairKernelSize, 0, velocityStream, properties.getFZeroPerMuZero(),
                    dips[(uint32_t)DIP::PAIR1], dips[(uint32_t)DIP::PAIR2], ddps[(uint32_t)DDP::RP], interval.x, lbb.x,
                    PBC_X == 1, ddps[(uint32_t)DDP::XP], ddps[(uint32_t)DDP::DXDTP], interval.y, lbb.y, PBC_Y == 1,
                    ddps[(uint32_t)DDP::YP], ddps[(uint32_t)DDP::DYDTP], interval.z, lbb.z, PBC_Z == 1,
                    ddps[(uint32_t)DDP::ZP], ddps[(uint32_t)DDP::DZDTP]);
      // Wall velocity
      ...
      if (USE_FLOW == 1)
      {
        ...
        KERNEL_LAUNCH(neighborVelocityKernel, pairKernelSize, 0, velocityStream, dips[(uint32_t)DIP::PAIR1],
                      dips[(uint32_t)DIP::PAIR2], numNeighbors, ddps[(uint32_t)DDP::TEMP1], ddps[(uint32_t)DDP::DXDTO],
                      ddps[(uint32_t)DDP::TEMP2], ddps[(uint32_t)DDP::DYDTO], ddps[(uint32_t)DDP::TEMP3],
                      ddps[(uint32_t)DDP::DZDTO]);

        KERNEL_LAUNCH(flowVelocityKernel, pairKernelSize, 0, velocityStream, numBubbles, numNeighbors,
                      ddps[(uint32_t)DDP::DXDTP], ddps[(uint32_t)DDP::DYDTP], ddps[(uint32_t)DDP::DZDTP],
                      ddps[(uint32_t)DDP::TEMP1], ddps[(uint32_t)DDP::TEMP2], ddps[(uint32_t)DDP::TEMP3],
                      ddps[(uint32_t)DDP::XP], ddps[(uint32_t)DDP::YP], ddps[(uint32_t)DDP::ZP],
                      properties.getFlowVel(), properties.getFlowTfr(), properties.getFlowLbb());
      }

      // Correct
      KERNEL_LAUNCH(correctKernel, kernelSize, 0, velocityStream, numBubbles, timeStep, ddps[(uint32_t)DDP::ERROR],
                    ddps[(uint32_t)DDP::XP], ddps[(uint32_t)DDP::X], ddps[(uint32_t)DDP::DXDT],
                    ddps[(uint32_t)DDP::DXDTP], ddps[(uint32_t)DDP::YP], ddps[(uint32_t)DDP::Y],
                    ddps[(uint32_t)DDP::DYDT], ddps[(uint32_t)DDP::DYDTP], ddps[(uint32_t)DDP::ZP],
                    ddps[(uint32_t)DDP::Z], ddps[(uint32_t)DDP::DZDT], ddps[(uint32_t)DDP::DZDTP]);
      ...
      KERNEL_LAUNCH(pathLengthDistanceKernel, kernelSize, 0, velocityStream, numBubbles, ddps[(uint32_t)DDP::TEMP4],
                    ddps[(uint32_t)DDP::PATH], ddps[(uint32_t)DDP::DISTANCE], ddps[(uint32_t)DDP::XP],
                    ddps[(uint32_t)DDP::X], ddps[(uint32_t)DDP::X0], dips[(uint32_t)DIP::WRAP_COUNT_XP], interval.x,
                    ddps[(uint32_t)DDP::YP], ddps[(uint32_t)DDP::Y], ddps[(uint32_t)DDP::Y0],
                    dips[(uint32_t)DIP::WRAP_COUNT_YP], interval.y, ddps[(uint32_t)DDP::ZP], ddps[(uint32_t)DDP::Z],
                    ddps[(uint32_t)DDP::Z0], dips[(uint32_t)DIP::WRAP_COUNT_ZP], interval.z);

      // Boundary wrap
      doBoundaryWrap(kernelSize, 0, velocityStream, PBC_X == 1, PBC_Y == 1, PBC_Z == 1, numBubbles,
                     ddps[(uint32_t)DDP::XP], ddps[(uint32_t)DDP::YP], ddps[(uint32_t)DDP::ZP], lbb, tfr,
                     dips[(uint32_t)DIP::WRAP_COUNT_X], dips[(uint32_t)DIP::WRAP_COUNT_Y],
      ...
      // Gas exchange
      KERNEL_LAUNCH(gasExchangeKernel, pairKernelSize, 0, gasExchangeStream, numBubbles, dips[(uint32_t)DIP::PAIR1],
                    dips[(uint32_t)DIP::PAIR2], ddps[(uint32_t)DDP::RP], ddps[(uint32_t)DDP::DRDTP],
                    ddps[(uint32_t)DDP::TEMP1], interval.x, PBC_X == 1, ddps[(uint32_t)DDP::XP], interval.y, PBC_Y == 1,
                    ddps[(uint32_t)DDP::YP], interval.z, PBC_Z == 1, ddps[(uint32_t)DDP::ZP]);
    }
    else // Two dimensions
    {
      // Reset
      KERNEL_LAUNCH(resetKernel, kernelSize, 0, 0, 0.0, numBubbles, ddps[(uint32_t)DDP::DXDTP],
                    ddps[(uint32_t)DDP::DYDTP], ddps[(uint32_t)DDP::DRDTP], ddps[(uint32_t)DDP::ERROR],
                    ddps[(uint32_t)DDP::TEMP1], ddps[(uint32_t)DDP::TEMP2]);
      ...
      // Boundary wrap
      doBoundaryWrap(kernelSize, 0, velocityStream, PBC_X == 1, PBC_Y == 1, false, numBubbles, ddps[(uint32_t)DDP::XP],
                     ddps[(uint32_t)DDP::YP], ddps[(uint32_t)DDP::ZP], lbb, tfr, dips[(uint32_t)DIP::WRAP_COUNT_X],
                     dips[(uint32_t)DIP::WRAP_COUNT_Y], dips[(uint32_t)DIP::WRAP_COUNT_Z],
                     dips[(uint32_t)DIP::WRAP_COUNT_XP], dips[(uint32_t)DIP::WRAP_COUNT_YP],
                     dips[(uint32_t)DIP::WRAP_COUNT_ZP]);
      ...
      KERNEL_LAUNCH(gasExchangeKernel, pairKernelSize, 0, gasExchangeStream, numBubbles, dips[(uint32_t)DIP::PAIR1],
                    dips[(uint32_t)DIP::PAIR2], ddps[(uint32_t)DDP::RP], ddps[(uint32_t)DDP::DRDTP],
                    ddps[(uint32_t)DDP::TEMP1], interval.x, PBC_X == 1, ddps[(uint32_t)DDP::XP], interval.y, PBC_Y == 1,
                    ddps[(uint32_t)DDP::YP]);
    }

    // Free area
    KERNEL_LAUNCH(freeAreaKernel, kernelSize, 0, gasExchangeStream, numBubbles, ddps[(uint32_t)DDP::RP],
                  ddps[(uint32_t)DDP::TEMP1], ddps[(uint32_t)DDP::TEMP2], ddps[(uint32_t)DDP::TEMP3]);

    cubWrapper.reduceNoCopy<double, double *, double *>(&cub::DeviceReduce::Sum, ddps[(uint32_t)DDP::TEMP1], dtfa,
                                                        numBubbles, gasExchangeStream);
    cubWrapper.reduceNoCopy<double, double *, double *>(&cub::DeviceReduce::Sum, ddps[(uint32_t)DDP::TEMP2], dtfapr,
                                                        numBubbles, gasExchangeStream);
    cubWrapper.reduceNoCopy<double, double *, double *>(&cub::DeviceReduce::Sum, ddps[(uint32_t)DDP::TEMP3], dta,
                                                        numBubbles, gasExchangeStream);

    KERNEL_LAUNCH(finalRadiusChangeRateKernel, kernelSize, 0, gasExchangeStream, ddps[(uint32_t)DDP::DRDTP],
                  ddps[(uint32_t)DDP::RP], ddps[(uint32_t)DDP::TEMP1], numBubbles, properties.getKappa(),
                  properties.getKParameter());

    // Radius correct
    KERNEL_LAUNCH(correctKernel, kernelSize, 0, gasExchangeStream, numBubbles, timeStep, ddps[(uint32_t)DDP::ERROR],
                  ddps[(uint32_t)DDP::RP], ddps[(uint32_t)DDP::R], ddps[(uint32_t)DDP::DRDT],
                  ddps[(uint32_t)DDP::DRDTP]);
    ...
    cubWrapper.reduceNoCopy<double, double *, double *>(&cub::DeviceReduce::Max, ddps[(uint32_t)DDP::RP],
                                                        static_cast<double *>(dtfa), numBubbles, gasExchangeStream);

    CUDA_CALL(
      cudaMemcpyAsync(static_cast<void *>(pinnedInts), mbpc, sizeof(int), cudaMemcpyDeviceToHost, gasExchangeStream));
    CUDA_CALL(cudaMemcpyAsync(static_cast<void *>(pinnedDoubles), dtfa, sizeof(double), cudaMemcpyDeviceToHost,
                              gasExchangeStream));
  ...
  CUDA_CALL(
    cudaMemcpyAsync(ddps[(uint32_t)DDP::DXDTO], ddps[(uint32_t)DDP::DXDT], numBytesToCopy, cudaMemcpyDeviceToDevice));
  CUDA_CALL(
    cudaMemcpyAsync(ddps[(uint32_t)DDP::X], ddps[(uint32_t)DDP::XP], 2 * numBytesToCopy, cudaMemcpyDeviceToDevice));
  CUDA_CALL(cudaMemcpyAsync(ddps[(uint32_t)DDP::PATH], ddps[(uint32_t)DDP::TEMP4], sizeof(double) * dataStride,
  ...
  continueSimulation &= (NUM_DIM == 3) ? maxBubbleRadius < 0.5 * (tfr - lbb).getMinComponent() : true;
...
void Simulator::updateCellsAndNeighbors()
{
  ...
  const size_t resetBytes = sizeof(int) * pairStride * ((uint64_t)DIP::NUM_VALUES - (uint64_t)DIP::PAIR1);
  CUDA_CALL(cudaMemset(dips[(uint32_t)DIP::PAIR1], 0, resetBytes));

  KernelSize kernelSize(128, numBubbles);

  KERNEL_LAUNCH(assignBubblesToCells, pairKernelSize, 0, 0, ddps[(uint32_t)DDP::X], ddps[(uint32_t)DDP::Y],
                ddps[(uint32_t)DDP::Z], cellIndices, bubbleIndices, properties.getLbb(), properties.getTfr(), cellDim,
                numBubbles);
  ...
  cubWrapper.histogram<int *, int, int, int>(&cub::DeviceHistogram::HistogramEven, cellIndices, sizes, maxNumCells + 1,
                                             0, maxNumCells, numBubbles);

  cubWrapper.scan<int *, int *>(&cub::DeviceScan::ExclusiveSum, sizes, offsets, maxNumCells);

  KERNEL_LAUNCH(
    reorganizeKernel, kernelSize, 0, 0, numBubbles, ReorganizeType::COPY_FROM_INDEX, sortedBubbleIndices,
    sortedBubbleIndices, ddps[(uint32_t)DDP::X], ddps[(uint32_t)DDP::XP], ddps[(uint32_t)DDP::Y],
    ddps[(uint32_t)DDP::YP], ddps[(uint32_t)DDP::Z], ddps[(uint32_t)DDP::ZP], ddps[(uint32_t)DDP::R],
    ddps[(uint32_t)DDP::RP], ddps[(uint32_t)DDP::DXDT], ddps[(uint32_t)DDP::DXDTP], ddps[(uint32_t)DDP::DYDT],
    ddps[(uint32_t)DDP::DYDTP], ddps[(uint32_t)DDP::DZDT], ddps[(uint32_t)DDP::DZDTP], ddps[(uint32_t)DDP::DRDT],
    ddps[(uint32_t)DDP::DRDTP], ddps[(uint32_t)DDP::DXDTO], ddps[(uint32_t)DDP::ERROR], ddps[(uint32_t)DDP::DYDTO],
    ddps[(uint32_t)DDP::TEMP1], ddps[(uint32_t)DDP::DZDTO], ddps[(uint32_t)DDP::TEMP2], ddps[(uint32_t)DDP::DRDTO],
    ddps[(uint32_t)DDP::TEMP3], ddps[(uint32_t)DDP::X0], ddps[(uint32_t)DDP::TEMP4], ddps[(uint32_t)DDP::Y0],
    ddps[(uint32_t)DDP::TEMP5], ddps[(uint32_t)DDP::Z0], ddps[(uint32_t)DDP::TEMP6], ddps[(uint32_t)DDP::PATH],
    ddps[(uint32_t)DDP::TEMP7], ddps[(uint32_t)DDP::DISTANCE], ddps[(uint32_t)DDP::TEMP8],
    dips[(uint32_t)DIP::WRAP_COUNT_X], dips[(uint32_t)DIP::WRAP_COUNT_XP], dips[(uint32_t)DIP::WRAP_COUNT_Y],
    dips[(uint32_t)DIP::WRAP_COUNT_YP], dips[(uint32_t)DIP::WRAP_COUNT_Z], dips[(uint32_t)DIP::WRAP_COUNT_ZP]);

  CUDA_CALL(cudaMemcpyAsync(static_cast<void *>(ddps[(uint32_t)DDP::X]), static_cast<void *>(ddps[(uint32_t)DDP::XP]),
                            memReqD / 2, cudaMemcpyDeviceToDevice));
  ...
  for (int i = 0; i < CUBBLE_NUM_NEIGHBORS + 1; ++i)
  {
    ...
    if (NUM_DIM == 3)
      KERNEL_LAUNCH(neighborSearch, kernelSize, 0, stream, i, numBubbles, maxNumCells, (int)pairStride, maxDistance,
                    offsets, sizes, dips[(uint32_t)DIP::TEMP1], dips[(uint32_t)DIP::TEMP2], ddps[(uint32_t)DDP::R],
                    interval.x, PBC_X == 1, ddps[(uint32_t)DDP::X], interval.y, PBC_Y == 1, ddps[(uint32_t)DDP::Y],
  ...
  cubWrapper.sortPairs<int, int>(&cub::DeviceRadixSort::SortPairs, const_cast<const int *>(dips[(uint32_t)DIP::TEMP1]),
...
void Simulator::deleteSmallBubbles(int numBubblesAboveMinRad)
{
  ...
  KERNEL_LAUNCH(calculateRedistributedGasVolume, kernelSize, 0, 0, ddps[(uint32_t)DDP::TEMP1], ddps[(uint32_t)DDP::R],
                dips[(uint32_t)DIP::FLAGS], numBubbles);

  cubWrapper.reduceNoCopy<double, double *, double *>(&cub::DeviceReduce::Sum, ddps[(uint32_t)DDP::TEMP1], dtv,
                                                      numBubbles);

  int *newIdx = dips[(uint32_t)DIP::TEMP1];
  cubWrapper.scan<int *, int *>(&cub::DeviceScan::ExclusiveSum, dips[(uint32_t)DIP::FLAGS], newIdx, numBubbles);

  KERNEL_LAUNCH(
    reorganizeKernel, kernelSize, 0, 0, numBubbles, ReorganizeType::CONDITIONAL_TO_INDEX, newIdx,
    dips[(uint32_t)DIP::FLAGS], ddps[(uint32_t)DDP::X], ddps[(uint32_t)DDP::XP], ddps[(uint32_t)DDP::Y],
    ddps[(uint32_t)DDP::YP], ddps[(uint32_t)DDP::Z], ddps[(uint32_t)DDP::ZP], ddps[(uint32_t)DDP::R],
    ddps[(uint32_t)DDP::RP], ddps[(uint32_t)DDP::DXDT], ddps[(uint32_t)DDP::DXDTP], ddps[(uint32_t)DDP::DYDT],
    ddps[(uint32_t)DDP::DYDTP], ddps[(uint32_t)DDP::DZDT], ddps[(uint32_t)DDP::DZDTP], ddps[(uint32_t)DDP::DRDT],
    ddps[(uint32_t)DDP::DRDTP], ddps[(uint32_t)DDP::DXDTO], ddps[(uint32_t)DDP::ERROR], ddps[(uint32_t)DDP::DYDTO],
...
{
  KERNEL_LAUNCH(transformPositionsKernel, pairKernelSize, 0, 0, normalize, numBubbles, properties.getLbb(),
...
void Simulator::saveSnapshotToFile()
{
  ...
  if (file.is_open())
  {
    ...
    CUDA_CALL(
      cudaMemcpy(hostData.data(), deviceDoubles, sizeof(double) * numComp * dataStride, cudaMemcpyDeviceToHost));
...
void Simulator::run(const char *inputFileName, const char *outputFileName)
{
  auto getVolumeOfBubbles = [this]() -> double {
    ...
    KERNEL_LAUNCH(calculateVolumes, kernelSize, 0, 0, ddps[(uint32_t)DDP::R], ddps[(uint32_t)DDP::TEMP1], numBubbles);

    return cubWrapper.reduce<double, double *, double *>(&cub::DeviceReduce::Sum, ddps[(uint32_t)DDP::TEMP1],
                                                         numBubbles);
  };
  ...
  std::cout << "\n=====\nSetup\n=====" << std::endl;
  {
    ...
    double t          = getVolumeOfBubbles() / (phiTarget * relativeSize.x * relativeSize.y * relativeSize.z);
  ...
  std::cout << "\n=============\nStabilization\n=============" << std::endl;
  {
    ...
    while (true)
    {
      ...
      {
        std::cout << (numSteps + 1) * properties.getNumStepsToRelax() << "\t" << deltaEnergy << "\t" << energy1 << "\t"
                  << energy2 << std::endl;
      }
  ...
  std::stringstream dataStream;
  {
    ...
    const size_t numBytesToCopy = 3 * sizeof(double) * dataStride;
    CUDA_CALL(cudaMemcpy(ddps[(uint32_t)DDP::X0], ddps[(uint32_t)DDP::X], numBytesToCopy, cudaMemcpyDeviceToDevice));
    ...
    while (continueIntegration)
    {
      ...
      if ((int)scaledTime >= timesPrinted)
      {
        ...
        energy2 = cubWrapper.reduce<double, double *, double *>(&cub::DeviceReduce::Sum, ddps[(uint32_t)DDP::TEMP4],
                                                                numBubbles);
        ...
        std::cout << (int)scaledTime << "\t" << getVolumeOfBubbles() / properties.getSimulationBoxVolume() << "\t"
                  << relativeRadius << "\t" << numBubbles << "\t" << dE << "\t" << numSteps << "\t" << numPairs
                  << std::endl;
